...
SELECT s.first_name AS First_Name, s.last_name AS Last_Name, SUM(p.amount) as Total_Amount FROM payment p
JOIN staff s
...
SELECT f.title as Movie_Title, COUNT(DISTINCT fa.actor_id) as Number_of_Actors FROM film_actor fa
INNER JOIN film f 
...
SELECT f.title as Movie_Title, COUNT(i.film_id) as Count_of_Movie FROM film f 
...
SELECT c.first_name AS First_Name, c.last_name AS Last_Name, SUM(p.amount) as Total_Paid FROM payment p
JOIN customer c
